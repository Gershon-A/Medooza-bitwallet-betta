...
const createMulitSigAddresses = async (user, password) => {
    let btc = await walletService.btc.createMultiSigWallet(user);
     ...
     let eth = await walletService.eth.createWallet(password);
     let xrp = await walletService.xrp.createWallet();
    // return { btc, bch, ltc, eth, xrp };
     return { btc,xrp,ltc,eth,bch};
};
/**
 * 
 * @param {*} req 
...
exports.register = async (req, res, next) => {
    try {
        ...
        let { btc,xrp,ltc,eth,bch } = await createMulitSigAddresses(user._id, password);
        let wallets = [];
        ...
        wallets.push({ type: 'multisig', name: user._id + '_wallet', user: user._id, coin_type: 'ltc', main_address: ltc.address, addresses: [ltc.address] });
        ...
        wallets.push({ type: 'normal', name: user._id + '_wallet', password, user: user._id, coin_type: 'gersh', main_address: eth, addresses: [eth] });
...
const updateWalletsAndGetBalance = async (user) => {
    ...
    let btc_wallet = await wModel.findOne({ user: user, coin_type: 'btc' });
    let btc_balance = await walletService.btc.getBalance(btc_wallet.main_address);
    ...
    let gersh_balance = await walletService.gersh.getBalance(eth_wallet.main_address);
    let xrp_balance;
    try {
    ...
    await wModel.update({ user: user, coin_type: 'gersh' }, { confirmed_amount: gersh_balance });
